...
import pathlib
install = pathlib.Path('deploy/install.sh').read_text()
required = ('running strict spectator TypeScript validation', '"$NEXT_SPECTATOR/server.ts"', '"$NEXT_SPECTATOR/state.ts"', '"$NEXT_SPECTATOR/controller-status.ts"', '"$NEXT_SPECTATOR/observer-watchdog.ts"', '"$NEXT_SPECTATOR/set-public-mission.ts"')
